...
import { useProductCategoryModule } from "~/composables/useProductCategoryModule";
import type {
  ProductCategory,
...
const emit = defineEmits<{
  success: [category: ProductCategory];
}>();

const { t } = useI18n();
const toast = useToast();
const { createProductCategory, updateProductCategory } =
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    const result = isUpdating.value
      ? await updateProductCategory(props.category!.id, payload)
...
<template>
  <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
    <UFormField :label="$t('product_categories.name')" name="name" required>
